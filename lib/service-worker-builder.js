...
const TREE_FOR_METHODS = {
  ...
  'service-worker-unregistration': 'treeForServiceWorkerUnRegistration',
};

const ENTRY_POINT_FILENAMES = {
  'service-worker': 'sw.js',
  'service-worker-registration': 'sw-registration.js',
  'service-worker-unregistration': 'sw-unregistration.js'
};
...
module.exports = class ServiceWorkerBuilder {
  ...
  _compileTrees(trees, treeName) {
    ...
    if ((treeName === 'sw-registration.js' || treeName === 'sw-unregistration.js') && registrationDistPath) {
  ...
  _rollupTree(tree, entryFile, destFile) {
    let rollupReplaceConfig = {
      include: [
        ...
        // same as above, but added equivalent regex to handle cases 
        // where the ember build cache paths can be different (ex: 3.4.x vs 3.16.x versions)
        /^(?:(?:^|\/|(?:(?:(?!(?:^|\/)\.{1,2}(?:\/|$)).)*?)\/)ember-service-worker(?:\/(?!\.{1,2}(?:\/|$))(?:(?:(?!(?:^|\/)\.{1,2}(?:\/|$)).)*?)\/|\/|$)(?!\.{1,2}(?:\/|$))(?=.)[^/]*?\.js)$/
      ],
